...
export const getPengurusList = async (params: PengurusParams) => {
  const response = await api.get<ApiListResponse<PengurusBackendRecord>>('/pengurus', {
...
export const getAnggotaDropdown = async (): Promise<Array<AnggotaOption>> => {
  const response = await api.get<ApiListResponse<BackendAnggota>>('/anggota/dropdown')
...
export const getJabatanDropdown = async (): Promise<Array<JabatanOption>> => {
  const response = await api.get<ApiListResponse<BackendJabatan>>('/jabatan/dropdown')
...
export const createPengurus = async (payload: PengurusUpsertPayload) => {
  ...
  try {
    const response = await api.post<ApiRecordResponse<PengurusBackendRecord>>('/pengurus', body)
...
export const updatePengurus = async (id: number, payload: PengurusUpsertPayload) => {
  ...
  try {
    const response = await api.put<ApiRecordResponse<PengurusBackendRecord>>(`/pengurus/${id}`, body)
    return mapBackendRecord(response.data.data)
  } catch (err) {
    handleApiError(err)
  }
...
export const deletePengurus = async (id: number) => {
  try {
    const response = await api.delete<{ status: string; message: string }>(`/pengurus/${id}`)
...
export const akhiriPengurus = async (id: number) => {
  try {
    const response = await api.patch<ApiRecordResponse<PengurusBackendRecord>>(`/pengurus/${id}/akhiri`)
    return mapBackendRecord(response.data.data)
  } catch (err) {
    handleApiError(err)
  }
