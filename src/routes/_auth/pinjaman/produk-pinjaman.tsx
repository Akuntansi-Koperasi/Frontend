...
import type { ProdukPinjamanListResult, ProdukPinjamanParams, ProdukPinjamanRecord } from '@/services/produkPinjamanService'
...
function RouteComponent() {
  ...
  const { canView, canManage, canDelete } = React.useMemo(() => getPermissionAccess('produkpinjaman'), [])
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['produk-pinjaman'] }),
  })

  const updateMutation = useMutation({
    mutationFn: ({ id, payload }: { id: number; payload: Omit<ProdukPinjamanRecord, 'id'> }) =>
  ...
  const normalizeApiErrors = (err: any, fallbackMessage: string): Partial<Record<string, Array<string>>> => {
  ...
  React.useEffect(() => {
    if (page < 1) {
      navigate({ to: '/pinjaman/produk-pinjaman', search: (prev: any) => ({ ...prev, page: 1 }), replace: true })
    }
  }, [navigate, page])
  ...
  const handleAdd = async (payload: Omit<ProdukPinjamanRecord, 'id'>) => {
    try {
      ...
      setAddErrors(normalizeApiErrors(err, 'Gagal menambahkan produk pinjaman'))
  ...
  const handleEdit = async (payload: ProdukPinjamanRecord) => {
    try {
      ...
      setEditErrors(normalizeApiErrors(err, 'Gagal memperbarui produk pinjaman'))
