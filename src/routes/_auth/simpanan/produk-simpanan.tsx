...
function RouteComponent() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['produk-simpanan'] }),
  })

  const updateMutation = useMutation({
    mutationFn: ({ id, payload }: { id: number; payload: Omit<ProdukSimpananRecord, 'id'> }) =>
  ...
  React.useEffect(() => {
    if (page < 1) {
      navigate({ to: '/simpanan/produk-simpanan', search: (prev: any) => ({ ...prev, page: 1 }), replace: true })
    }
  }, [navigate, page])
  ...
  const handleAdd = async (payload: Omit<ProdukSimpananRecord, 'id'>) => {
    try {
      ...
      setAddErrors(normalizeApiErrors(err, 'Gagal menambahkan produk simpanan'))
  ...
  const handleEdit = async (payload: ProdukSimpananRecord) => {
    try {
      ...
      setEditErrors(normalizeApiErrors(err, 'Gagal memperbarui produk simpanan'))
