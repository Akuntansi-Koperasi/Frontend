...
export function PengurusEditDialog({ open, onOpenChange, pengurus, onEdit }: PengurusEditDialogProps) {
  ...
  useEffect(() => {
    if (open && pengurus) {
      setAnggotaId(pengurus.anggotaId)
      setJabatan(pengurus.jabatan)
      setMulaiMenjabat(pengurus.mulaiMenjabat.toString())
      setSelesaiMenjabat(pengurus.selesaiMenjabat.toString())
  ...
  const isFormValid = useMemo(
    () =>
      ...
      Boolean(pengurus),
    [anggotaId, jabatan, mulaiMenjabat, selesaiMenjabat, pengurus]
  )

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!pengurus || !isFormValid || !selectedAnggota) return
